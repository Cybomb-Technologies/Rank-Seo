...
export default function DashboardPage() {
  ...
  const [serviceData, setServiceData] = useState<
    Record<ServiceType, ServiceData>
  >({
    all: {
      service: "all",
      label: "All Services",
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const auditsData = (await auditsRes.ok)
          ? await auditsRes.json()
          : { audits: [] };
        ...
        const businessData = (await businessRes.ok)
        ...
        const keywordData = (await keywordRes.ok)
        ...
        const keycheckerData = (await keycheckerRes.ok)
          ? await keycheckerRes.json()
          : { data: [] };
        ...
        const scraperData = (await scraperRes.ok)
          ? await scraperRes.json()
          : { data: [] };

        // Process SEO Audit Data
        const auditItems = (
          Array.isArray(auditsData) ? auditsData : auditsData.audits || []
        ).map((audit: any) => ({
        ...
        const keycheckerItems = (keycheckerData.data || []).map(
          (report: any) => ({
          ...
          "seo-audit": {
            ...
            stats: {
              ...
              averageScore:
                auditItems.length > 0
                  ? Math.round(
                      auditItems.reduce((sum, item) => sum + item.score, 0) /
                        auditItems.length
                    )
                  : 0,
            },
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            {selectedService !== "all" && serviceLinks[selectedService] && (
              <Link
                href={serviceLinks[selectedService]}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={currentService.chartData}>
                    ...
                    <Bar
                      ...
                      name={
                        selectedService === "seo-audit" ? "SEO Score" : "Count"
                      }
                    />
          ...
          {selectedService !== "all" && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Key metrics and actions for{" "}
                  {currentService.label.toLowerCase()}
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {currentService.stats.total > 0
                          ? `${Math.round(
                              (currentService.recentItems.filter(
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`text-sm font-bold ${
                              currentService.stats.averageScore >= 80
                                ? "text-green-600"
                                : currentService.stats.averageScore >= 60
                                ? "text-yellow-600"
                                : "text-red-600"
                            }`}
                          >
                  ...
                  {selectedService === "keyword-scraper" && (
                    <div className="p-3 rounded-lg bg-muted/50">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {currentService.recentItems
                            .reduce((sum, item) => sum + item.secondaryScore, 0)
                            .toLocaleString()}
                        </span>
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          ...
                          {selectedService === "business-names" &&
                            "Generate names for different industries to expand your portfolio."}
                          {selectedService === "keyword-reports" &&
                            "Compare keyword trends across different time periods."}
                          {selectedService === "keyword-checker" &&
                            "Analyze competitor keywords to identify new opportunities."}
                          {selectedService === "keyword-scraper" &&
                            "Scrape multiple pages to get comprehensive keyword data from websites."}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 text-gray-600 text-sm">
                    ...
                    {selectedService === "keyword-scraper" && (
                      <th className="py-3 px-4">KEYWORDS FOUND</th>
                    )}
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <div
                              ...
                              style={{
                                backgroundColor: serviceData[item.type].color,
                              }}
                            />
                            {item.name.length > 30
